...
serve(async (req) => {
  ...
  try {
    ...
    if (contextParts.length === 0) {
      return new Response(JSON.stringify({ error: "Not enough profile data to generate a bio. Please fill in some sections first." }), {
        status: 400,
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a professional profile writer. Generate a concise, compelling profile overview / bio (3-5 sentences) based on the user's profile data. The bio should:
- Be written in third person
- Provide a high-level overview of who the person is professionally
- Highlight their expertise, natural role, and key strengths
- Be professional yet engaging
- NOT include any headers, labels, or quotes — just the bio text itself`
          },
          {
            role: "user",
            content: `Generate a professional profile overview based on this data:\n\n${contextParts.join("\n")}`
          }
        ],
      }),
    });
