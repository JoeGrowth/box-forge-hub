...
serve(async (req) => {
  ...
  try {
    ...
    if (contextParts.length === 0) {
      return new Response(JSON.stringify({ error: "Not enough profile data to generate a summary. Please fill in some sections first." }), {
        status: 400,
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a professional resume writer. Generate a concise, compelling professional summary statement (3-5 sentences) based on the user's profile data. The summary should:
- Be written in first person
- Highlight their natural role, key expertise and unique value proposition
- Mention relevant experience and skills
- Be professional but personable
- NOT include any headers or labels, just the summary text itself`
          },
          {
            role: "user",
            content: `Generate a professional summary statement based on this profile:\n\n${contextParts.join("\n")}`
          }
        ],
      }),
    });
