...
DECLARE
  ...
  r record;
  scores jsonb := '[]'::jsonb;
  qualifying text[] := ARRAY['explorer']::text[];
  best_state text;
  best_sort  int;
  runner_state text;
  reasons jsonb := '[]'::jsonb;
...
BEGIN
  SELECT COALESCE(expertise_score,0), COALESCE(verified_expertise_count,0)
    INTO v_exp_raw, v_verified FROM public.expertise_graph WHERE user_id = _user_id;
  SELECT COALESCE(trust_score,0) INTO v_trust_raw FROM public.trust_graph WHERE user_id = _user_id;
  SELECT COALESCE(completed_value_count,0), COALESCE(total_revenue,0)
    INTO v_tx, v_revenue FROM public.revenue_graph WHERE user_id = _user_id;
  SELECT COALESCE(total_allocated_equity,0), COALESCE(venture_count,0)
    INTO v_own, v_ventures FROM public.ownership_graph WHERE user_id = _user_id;
  ...
  IF v_cold_conf >= 0.6 OR v_verified >= 1 OR v_exp_raw >= 10 OR v_learning >= 1 OR v_apps >= 3 THEN
    qualifying := qualifying || ARRAY['builder']::text[];
  END IF;
  IF v_verified >= 2 OR (v_verified >= 1 AND v_trust_raw >= 20) OR v_trust_raw >= 50 THEN
    qualifying := qualifying || ARRAY['validated_expert']::text[];
  END IF;
  IF v_revenue >= 500 OR v_consulting >= 1 OR (v_tx >= 1 AND v_revenue > 0) THEN
    qualifying := qualifying || ARRAY['professional_operator']::text[];
  END IF;
  IF v_own > 0 THEN
    qualifying := qualifying || ARRAY['co_builder']::text[];
  END IF;
  IF v_ventures >= 1 AND v_own >= 15 THEN
    qualifying := qualifying || ARRAY['venture_creator']::text[];
  END IF;

  FOR r IN SELECT * FROM public.professional_state_weights ORDER BY sort_order
  LOOP
    DECLARE s numeric; gated boolean := NOT (r.state = ANY(qualifying));
    BEGIN
      ...
      scores := scores || jsonb_build_object('state', r.state, 'score', ROUND(s,2), 'eligible', NOT gated);
    END;
  END LOOP;
  ...
  IF v_tx >= 1 AND v_revenue > 0 THEN reasons := reasons || jsonb_build_array(format('%s completed engagement(s)', v_tx)); END IF;
