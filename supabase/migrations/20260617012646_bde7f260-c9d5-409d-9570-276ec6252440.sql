...
BEGIN
  ...
  DELETE FROM auth.users WHERE email LIKE 'sim_%@beta.local';

  INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password,
    email_confirmed_at, raw_app_meta_data, raw_user_meta_data, created_at, updated_at)
  SELECT uid, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
    'sim_'||rn::text||'_'||replace(uid::text,'-','')||'@beta.local',
    pw, now(), '{"provider":"email","providers":["email"]}'::jsonb,
    jsonb_build_object('full_name','Sim '||rn,'role','cobuilder'), now(), now()
  ...
  FOREACH u IN ARRAY uids LOOP PERFORM public.seed_cold_start_expertise(u); END LOOP;
  ...
  PERFORM 1 FROM (
    WITH un AS (SELECT u_, public.graph_upsert_node('user'::graph_node_type, u_::text, NULL, '{}'::jsonb) nid
                FROM unnest(ARRAY[uids[5],uids[6],uids[7],uids[8]]) u_),
         cn AS (SELECT u_, public.graph_upsert_node('certification'::graph_node_type,
                  'sim_cert_'||u_::text||'_'||g, 'Sim Cert '||g, '{}'::jsonb) cnid
                FROM unnest(ARRAY[uids[5],uids[6],uids[7],uids[8]]) u_, generate_series(1,3) g)
    SELECT public.graph_upsert_edge(un.nid, cn.cnid, 'HAS_CERTIFICATION'::graph_edge_type, 1.0,
                                    '{"verified":true}'::jsonb, NULL, now()) x
    FROM un JOIN cn ON cn.u_ = un.u_) sub;

  PERFORM 1 FROM (
    WITH un AS (SELECT u_, public.graph_upsert_node('user'::graph_node_type, u_::text, NULL, '{}'::jsonb) nid
                FROM unnest(ARRAY[uids[7],uids[8]]) u_),
         rn AS (SELECT u_, public.graph_upsert_node('review'::graph_node_type,
                  'sim_rev_'||u_::text||'_'||g, 'rev', '{}'::jsonb) rnid
                FROM unnest(ARRAY[uids[7],uids[8]]) u_, generate_series(1,3) g)
    SELECT public.graph_upsert_edge(un.nid, rn.rnid, 'USER_RECEIVED_REVIEW'::graph_edge_type, 1.0,
                                    jsonb_build_object('rating', 4.5), NULL, now()) x
    FROM un JOIN rn ON rn.u_ = un.u_) sub;
  ...
  SELECT t.seller_id, 'transaction_completed'::graph_event_type, 1, 'transaction', t.id::text, 'simulation',
         'sim_tx_completed:'||t.id::text,
         jsonb_build_object('transaction_id', t.id::text, 'amount', t.amount, 'seller_id', t.seller_id::text), 1, t.completed_at
  ...
  SELECT ea.id, ea.percentage, 48, 12, 'monthly' FROM public.equity_allocations ea
   WHERE ea.source = 'sim'
     AND NOT EXISTS (SELECT 1 FROM public.vesting_schedules vs WHERE vs.equity_allocation_id = ea.id);

  INSERT INTO public.graph_events (user_id, event_type, event_version, aggregate_type, aggregate_id,
    source_module, idempotency_key, payload, weight, occurred_at)
  SELECT ea.user_id, 'equity_allocation_created'::graph_event_type, 1, 'equity_allocation', ea.id::text, 'simulation',
         'sim_equity_alloc:'||ea.id::text,
         jsonb_build_object('allocation_id', ea.id::text, 'venture_id', ea.venture_id::text, 'percentage', ea.percentage), 1, now()
