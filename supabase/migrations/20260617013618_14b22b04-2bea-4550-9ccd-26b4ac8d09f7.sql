...
BEGIN
  ...
  SELECT COUNT(*) INTO v_intent    FROM public.graph_events
    WHERE user_id = _user_id AND event_type IN ('intent_declared','intent_signal_recorded','goal_created','onboarding_completed','cold_start_confirmed');
  ...
  LOOP
    DECLARE s numeric; gated boolean := false;
    BEGIN
      ...
      ELSE
        ...
        s := COALESCE(s, 0);
      END IF;
      scores := scores || jsonb_build_object('state', r.state, 'score', ROUND(s,2), 'gated', gated);
