        ...
        <main className="pt-20">
          ...
          <section className="py-12">
            <div className="container max-w-5xl mx-auto px-4">
              ...
              <div className="mb-10">
                <div className="mb-4">
                  <h2 className="text-xl font-display font-bold text-foreground">Your Certifications</h2>
                  <p className="text-sm text-muted-foreground">Pick up where you left off, or start a new path.</p>
                </div>
                <div className="space-y-3">
                  {([
                    { key: "initiator" as ActiveSection, title: "Learn to be an Initiator", icon: Lightbulb, total: INITIATOR_STEPS.length },
                    ...
                    const journey = journeys.find(j => j.journey_type === journeyType);
                    const certified = isSectionCertified(key);
                    const phase = journey?.current_phase ?? 0;
                    const started = !!journey && (journey.status !== "not_started");
                    let state: "done" | "continue" | "start" = "start";
                    if (certified || journey?.status === "approved") state = "done";
                    else if (started || phase > 0 || journey?.status === "pending_approval") state = "continue";

                    const badge =
                      state === "done"
                        ? <Badge className="bg-b4-navy text-white">Completed</Badge>
                        : state === "continue"
                        ? <Badge className="bg-b4-teal text-white">In Progress</Badge>
                        : <Badge variant="outline">Not Started</Badge>;

                    const ctaLabel = state === "done" ? "Review" : state === "continue" ? "Continue" : "Start";

                    return (
                      <div
                        key={key}
                        className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${
                          state === "done" ? "bg-b4-teal/5 border-b4-teal/30" : "bg-muted/30 border-border/60 hover:border-b4-teal/40"
                        }`}
                      >
                        <div className={`flex items-center justify-center w-10 h-10 rounded-full flex-shrink-0 ${
                          state === "done" ? "bg-b4-teal text-white" : "bg-background border border-border text-muted-foreground"
                        }`}>
                          {state === "done" ? <CheckCircle className="w-5 h-5" /> : <Award className="w-5 h-5" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-sm text-muted-foreground">
                            Phase {Math.min(phase, total)} of {total}
                          </div>
                        </div>
                        <Button
                          size="sm"
